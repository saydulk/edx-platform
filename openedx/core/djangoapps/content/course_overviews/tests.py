...
class CourseOverviewTestCase(ModuleStoreTestCase):
    ...
    def test_get_errored_course(self, modulestore_type):
        ...
        """
        course = CourseFactory.create(default_store=modulestore_type)
        #store = modulestore()._get_modulestore_by_type(modulestore_type)  # pylint: disable=unused-variable
        with mock.patch('xmodule.modulestore.mixed.MixedModuleStore.get_course', mock.Mock(return_value=ErrorDescriptor)):
